...
### 4. GeobugiGlamGaming
An Angular 21 e-commerce style catalog for a video game store, built with standalone components and SSR support.
* **Applied Concepts:** Component-based architecture with a shared `ProductoTarjeta` (product card) component, typed data modeling via the `Videojuego` interface, route-based page composition (`Inicio`, `Productos`, `Ofertas`), and client-side filtering of a product catalog.
* **Key Features:** Home, catalog (`/productos`) and deals (`/ofertas`) pages rendering a game catalog from a shared data source, with the offers page filtering games currently on sale (`precioOferta`); Angular Material theming integration.
